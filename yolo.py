...
INSTANCE_FOLDER = "logs/2/save_fig"
...
class YOLO(object):
    ...
    def detect_16bit_image(self, image):
        ...
        if self.letterbox_image:
            ...
            # crop_img = image.convert('RGB')
            # crop_img = crop_img.resize((self.model_image_size[1], self.model_image_size[0]), Image.BICUBIC)
            crop_img = cv2.resize(image, (self.model_image_size[1], self.model_image_size[0]),
                                  interpolation=cv2.INTER_NEAREST)

        photo = np.array(crop_img, dtype=np.float32) / self.max_val
        photo = np.transpose(photo, (2, 0, 1))
        # ---------------------------------------------------------#
        #   添加上batch_size维度
        # ---------------------------------------------------------#
